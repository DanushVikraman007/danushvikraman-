...
export function applyMode(mode) {
  ...
  document.documentElement.dataset.mode = isLight ? 'light' : 'dark';

  // Mobile browser chrome (status bar / safe-area strip) reads this meta
  // tag, not any CSS var — without updating it, the strip stays pinned to
  // whatever was hardcoded in index.html regardless of the active theme,
  // which is what produced the stray dark sliver in light mode.
  ...
  // Prefer the live --bg token if it's already been set by applyBg this
  // theme; fall back to sensible per-mode defaults on first paint.
  const liveBg = getComputedStyle(document.documentElement).getPropertyValue('--bg').trim();
  meta.content = liveBg || (isLight ? '#ecebe6' : '#050505');
}
...
export function applyTheme(t) {
  ...
  // mode last: it reads the live --bg to sync the mobile browser-chrome
  // meta tag, so bg/accent must already be applied by this point.
