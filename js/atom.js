...
export function mountAtom(canvas, hud = {}) {
  ...
  function frame(now) {
    ...
    for (const c of cloud) {
      ...
      // Slightly larger than before (was 1.05/1.25) — dots read as small
      // spheres rather than dust at this size, without turning the cloud
      // into blobs when it's dense near the core.
      const size = (c.core ? 1.5 : 1.75) * q.s;
      // Shaded instead of flat: a dim "shadow" square fills the dot, with
      // a small bright "glint" in one corner on top. Two flat fillRects
      // stay cheap at ~3500 particles/frame — a per-particle radial
      // gradient (like the nucleus uses) would be far too costly at this
      // count — but the glint-on-shadow pair reads as a lit, rounded
      // surface rather than a flat pixel. The glint is deliberately small
      // (0.42x) so the darker shadow tone dominates the dot's visible
      // area — an equal-size highlight on top of the shadow just paints
      // over most of it and nets out brighter, not darker.
      const shadowMul = 0.38; // darker overall, per the "lil dark" ask
      const hiMul = 1.3;
      const glintSize = size * 0.42;
      const glintOff = size * 0.2;
      if (c.core) {
        // hotter, whiter core
        const hr = Math.min(255, r + 60), hg = Math.min(255, g + 60), hb = Math.min(255, b + 50);
        ctx.fillStyle = `rgba(${Math.floor(hr * shadowMul)},${Math.floor(hg * shadowMul)},${Math.floor(hb * shadowMul)},${a})`;
        ctx.fillRect(q.x - size / 2, q.y - size / 2, size, size);
        ctx.fillStyle = `rgba(${Math.min(255, hr * hiMul)},${Math.min(255, hg * hiMul)},${Math.min(255, hb * hiMul)},${a})`;
        ctx.fillRect(q.x - size / 2 - glintOff, q.y - size / 2 - glintOff, glintSize, glintSize);
      } else {
        ctx.fillStyle = `rgba(${Math.floor(r * shadowMul)},${Math.floor(g * shadowMul)},${Math.floor(b * shadowMul)},${a})`;
        ctx.fillRect(q.x - size / 2, q.y - size / 2, size, size);
        ctx.fillStyle = `rgba(${Math.min(255, r * hiMul)},${Math.min(255, g * hiMul)},${Math.min(255, b * hiMul)},${a})`;
        ctx.fillRect(q.x - size / 2 - glintOff, q.y - size / 2 - glintOff, glintSize, glintSize);
      }
    }
